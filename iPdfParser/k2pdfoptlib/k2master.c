...
#ifdef kContinuousReadSinglePageMode
    int heightFrom=masterinfo->rows;
    int heightTo=masterinfo->rows+tmp->height;
    int devHeight=k2settings->dev_height;
    int paddingHeight=getPaddingHeight(heightFrom, heightTo,devHeight);
    if(paddingHeight>0){
        unsigned char *pdst;
        pdst=bmp_rowptr_from_top(&masterinfo->bmp,masterinfo->rows);
        memset(pdst,255,bmp_bytewidth(&masterinfo->bmp)*paddingHeight);
        masterinfo->rows += paddingHeight;
    }
#endif
#if (WILLUSDEBUGX & 32)
printf("JJ\n");
