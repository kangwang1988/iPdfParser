...
enum { FINSIDE, FOUTSIDE, FLEAVE, FENTER };
...
fz_insert_gel(fz_gel *gel, float fx0, float fy0, float fx1, float fy1)
{
	...
	if (d == FOUTSIDE) return;
	if (d == FLEAVE) { y1 = gel->clip.y0; x1 = v; }
	if (d == FENTER) { y0 = gel->clip.y0; x0 = v; }

	d = clip_lerp_y(gel->clip.y1, 1, x0, y0, x1, y1, &v);
	if (d == FOUTSIDE) return;
	if (d == FLEAVE) { y1 = gel->clip.y1; x1 = v; }
	if (d == FENTER) { y0 = gel->clip.y1; x0 = v; }
